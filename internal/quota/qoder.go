...
func (p qoderProvider) Check(ctx context.Context, input ProviderInput) (ProviderOutput, error) {
	...
	if !ok { return ProviderOutput{}, fmt.Errorf("Qoder credits endpoint is unavailable") }
	body, err := caller.FetchQoderCredits(ctx, input.Identity.Identity)
	if err != nil { return ProviderOutput{}, err }
	var payload struct { Accounts []qoderCreditsAccount `json:"accounts"` }
	if err := json.Unmarshal(body, &payload); err != nil { return ProviderOutput{}, err }
	for _, account := range payload.Accounts {
		if account.AuthIndex != input.Identity.Identity { continue }
		if account.Error != "" { return ProviderOutput{}, fmt.Errorf("Qoder credits: %s", account.Error) }
		if account.Credits == nil || len(account.Credits.Packages) == 0 { return ProviderOutput{}, fmt.Errorf("Qoder credits packages are missing") }
		for _, pack := range account.Credits.Packages {
			if pack.Remain == nil || pack.Used == nil || (pack.SizeKnown && (pack.Size == nil || *pack.Size < 0)) { return ProviderOutput{}, fmt.Errorf("Qoder credits package values are missing or invalid") }
		}
...
func (c upstreamResponseRecordingCaller) FetchQoderCredits(ctx context.Context, authIndex string) ([]byte, error) {
	caller, ok := c.caller.(qoderCreditsCaller)
	if !ok { return nil, fmt.Errorf("Qoder credits endpoint is unavailable") }
...
func normalizeQoderQuotaRows(account qoderCreditsAccount) []QuotaRow {
	...
	if account.Credits == nil { return rows }
	for index, pack := range account.Credits.Packages {
		available := pack.Available
		if expiry, err := time.Parse(time.RFC3339, pack.CycleEnd); err == nil && !expiry.After(time.Now()) { available = false }
		...
		if pack.SizeKnown { row.Limit = pack.Size }
