...
func (caller qoderTestCaller) CallManagementAPI(context.Context, apicall.Request) (*apicall.Response, error) {
	panic("must use plugin credits endpoint")
}
func (caller qoderTestCaller) FetchQoderCredits(context.Context, string) ([]byte, error) {
	return []byte(caller.body), nil
}

func (caller qoderTestCaller) ResetQuota(context.Context, string) error {
	panic("credit refresh must not reset quota")
}
...
func TestQoderCredits(t *testing.T) {
	...
	provider := qoderProvider{caller: upstreamResponseRecordingCaller{ManagementClient: caller}}
	...
	if len(rows) != 3 || *rows[0].Remaining != 0 || *rows[1].Remaining != 19.5 || *rows[1].Allowed || rows[1].ResetAt != "" || rows[1].ExpiresAt == "" || rows[2].Limit != nil || rows[2].UsedPercent != nil {
	...
	if _, ok := decoded["tasks"].([]any); !ok {
		t.Fatalf("tasks must be array: %s", encoded)
	}
}
